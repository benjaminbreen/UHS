...
export function namedShrub(
  ...
  const named =
    sprite === "nature-understory-fungi"
      ? `nature-fungus-${species}`
      : `nature-shrub-${species}`;
...
export function foliageTint(
  ...
  if (season === "autumn")
    return autumnTints[
      Math.floor(variation * autumnTints.length) % autumnTints.length
    ];
...
export function driftStyle(
  ...
  if (season === "summer") return drift("seed", ["#f2eeda", "#e8e4c4"], 40);
  return undefined;
}
