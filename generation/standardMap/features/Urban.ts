/**
 * generation/standardMap/features/urban.ts - Urban generation logic for Standard Maps
 */
...
function generateUrbanCluster(tiles: Tile[][], center: Point, clusterIndex: number, totalClusters: number, generateLargeCity: boolean | undefined, randomNoise: ValueNoise): void {
  ...
  let denseCityTiles = Math.floor((DENSE_CITY_SIZE_MIN + randomNoise.random() * (DENSE_CITY_SIZE_MAX - DENSE_CITY_SIZE_MIN)) * cityScaleFactor * 0.4);
  let lowDensityTiles = Math.floor((LOW_DENSITY_CITY_SIZE_MIN + randomNoise.random() * (LOW_DENSITY_CITY_SIZE_MAX - LOW_DENSITY_CITY_SIZE_MIN)) * cityScaleFactor * 1.2 * 0.5);
  let hamletTiles = Math.floor((HAMLET_SIZE_MIN + randomNoise.random() * (HAMLET_SIZE_MAX - HAMLET_SIZE_MIN)) * cityScaleFactor * 1.3);
...
export function generateUrbanAreas(tiles: Tile[][], randomNoise: ValueNoise, archetype: MapArchetype, harborSide?: number, generateLargeCity?: boolean) {
  ...
  clusterCenters.forEach((center, index) => {
    generateUrbanCluster(tiles, center, index, clusterCenters.length, generateLargeCity, randomNoise);
  });
