...
export function generateGovernmentForum(
  ...
  if (config.culturalZone === 'EUROPEAN') {
    if (config.era === HistoricalEra.ANTIQUITY) {
      generateRomanSenate(tiles, config, noise, size, interactionZones);
    } else if (config.era === HistoricalEra.MEDIEVAL) {
      generateMedievalCouncilChamber(tiles, config, noise, size, interactionZones);
    } else if (config.era === HistoricalEra.RENAISSANCE_EARLY_MODERN) {
      generateRenaissanceParliament(tiles, config, noise, size, interactionZones);
    } else if (config.era === HistoricalEra.INDUSTRIAL_ERA || config.era === HistoricalEra.MODERN_ERA) {
      generateModernParliament(tiles, config, noise, size, interactionZones);
    } else {
...
function generateRomanSenate(
  ...
  for (let x = 8; x < size.width - 8; x += 3) {
    tiles[platformY + 2][x].biome = BiomeType.THRONE;
    tiles[platformY + 2][x].materialSubtype = 'curule_chair';
  }
...
function generateMedievalCouncilChamber(
  ...
  // Lord's throne at head of table
  tiles[tableY - 3][centerX].biome = BiomeType.THRONE;
  tiles[tableY - 3][centerX].materialSubtype = 'high_seat';
  ...
  // Fireplace/hearth
  tiles[5][centerX].biome = BiomeType.BRAZIER;
  tiles[5][centerX].materialSubtype = 'great_hearth';
...
function generateChineseImperialCourt(
  ...
  // Dragon throne
  tiles[10][centerX].biome = BiomeType.THRONE;
  tiles[10][centerX].materialSubtype = 'dragon_throne';
  ...
  // Incense burners
  tiles[12][centerX - 4].biome = BiomeType.BRAZIER;
  tiles[12][centerX - 4].materialSubtype = 'incense_burner';
  tiles[12][centerX + 4].biome = BiomeType.BRAZIER;
  tiles[12][centerX + 4].materialSubtype = 'incense_burner';
...
function generateJapaneseDaimyoHall(
  ...
  tiles[size.height - 5][6].biome = BiomeType.BRAZIER;
  tiles[size.height - 5][6].materialSubtype = 'tea_brazier';
...
function generateIslamicMajlis(
  ...
  // Caliph/Sultan's seat
  tiles[3][centerX].biome = BiomeType.THRONE;
  tiles[3][centerX].materialSubtype = 'cushioned_throne';
  
  // Braziers for incense
  tiles[6][8].biome = BiomeType.BRAZIER;
  tiles[6][8].materialSubtype = 'incense_brazier';
  tiles[6][size.width - 9].biome = BiomeType.BRAZIER;
  tiles[6][size.width - 9].materialSubtype = 'incense_brazier';
...
function generateAfricanCouncilGround(
  ...
  // Central fire pit
  tiles[centerY][centerX].biome = BiomeType.BRAZIER;
  tiles[centerY][centerX].materialSubtype = 'fire_pit';
  ...
  // Chief's seat (elevated)
  tiles[centerY - 8][centerX].biome = BiomeType.THRONE;
  tiles[centerY - 8][centerX].materialSubtype = 'leopard_skin_throne';
...
function generateIndianDurbar(
  ...
  // Peacock throne or gaddi
  tiles[platformY + 2][centerX].biome = BiomeType.THRONE;
  ...
  // Incense holders
  tiles[platformY + 1][platformX - 1].biome = BiomeType.BRAZIER;
  tiles[platformY + 1][platformX - 1].materialSubtype = 'silver_incense';
  tiles[platformY + 1][platformX + platformWidth].biome = BiomeType.BRAZIER;
  tiles[platformY + 1][platformX + platformWidth].materialSubtype = 'silver_incense';
...
function generateAsianCouncilHall(
  ...
  tiles[5][centerX].biome = BiomeType.THRONE;
...
function generateRenaissanceParliament(
  ...
  // Speaker's chair
  tiles[5][centerX].biome = BiomeType.THRONE;
  tiles[5][centerX].materialSubtype = 'speaker_chair';
