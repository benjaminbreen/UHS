...
Same-chunk browser comparisons across eight Roman city chunks produced identical ground-page pixels, reduced `putImageData` calls from 2,048 to 8, and measured average ground installation around 0.6 ms versus 8.9–9.6 ms on the old path. These are local ground-stage measurements, not total load-time guarantees. Installation metrics now exclude worker dispatch/creation and separately expose ground installation time and batched tile count. Six focused unit tests and TypeScript checks pass. The browser comparison also exercises subsequent camera streaming. Reviewed `artifacts/ground-pages.png`. The older terrain-world browser suite was interrupted at its obsolete “New world” startup selector, before terrain actions ran. Batching changes are not committed or pushed.
...
The panel now also exposes composition controls that materially change the scene: deterministic rock-redistribution previews with density, altitude, dryness, clustering and cluster-scale parameters; ground-mark density, spacing and quiet-patch contrast; and rerasterized path width, center-line wander, edge breakup and verge tufts. These remain visual studies so collision, route geometry and saved worlds stay untouched. A vegetation palette can replace ordinary world trees with two `trees.png` trees or the mature frames from the oak, birch, cedar, fir, hazel, maple, willow, apple or cherry growth strips in `trees pngs/`, with a live scale control. Woodland logs/brush and grassland tufts from `trees.png` can be scattered as an independent, density-controlled study layer; its white source background is keyed transparent at load time. Production build and the focused browser check pass with the new asset bundle and live swaps.
