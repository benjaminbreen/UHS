...
302c17dd8b2d7c809c7056ee9f7f8ed973a8dffa. Its regional coordinates are NOT used.
Unmatched legacy names are left out rather than assigned misleading coordinates.
"""
import json, re, unicodedata, urllib.request, hashlib
...
for f in sorted(cities,key=lambda f:f['properties']['pop_max'] or 0):
 p=f['properties'];point=(p['longitude'],p['latitude'])
 for k in ['name','nameascii','namealt']:
  if p.get(k):locations[norm(p[k])]=point
for f in regions:
 ...
 for k in ['NAME','NAME_EN','NAMEALT']:
  if p.get(k):locations.setdefault(norm(p[k]),point)
...
for p in legacy:
 ...
 point=next((locations[n] for n in names if n in locations),None)
 if point and abs(point[1])<=85:
  lon,lat=point
  result[norm(p['name'])]={**p,**defaults(lon,lat),'climate':p['climate'],'relief':p['relief'],'water':p['water'],'lon':round(lon,4),'lat':round(lat,4)};matched+=1
for f in sorted(cities,key=lambda f:-(f['properties']['pop_max'] or 0)):
 ...
 result[key]=dict(id='city-'+key.replace(' ','-'),name=name,aliases=[p['nameascii']] if p.get('nameascii')!=name and p.get('nameascii') else [],lon=round(lon,4),lat=round(lat,4),**defaults(lon,lat))
