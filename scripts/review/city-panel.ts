...
const rows: string[] = [
  "| city | year | radius | buildings | people | built share | parks | gen ms | streets | buildings | routines |",
  "|---|---|---|---|---|---|---|---|---|---|---|",
];
for (const { place, year } of PANEL) {
  ...
  const t = c.timing;
  rows.push(
    `| ${place} | ${year} | ${c.radius} | ${c.buildings} | ${c.humans} | ${(c.builtShare * 100).toFixed(1)}% | ${c.parks} | ${c.genMs} | ${t.streets ?? ""} | ${t.buildings ?? ""} | ${t.routines ?? ""} |`,
  );
  console.log(rows.at(-1));
  if (capture)
    execFileSync("npx", ["playwright", "test", "tests/browser/city-capture.spec.ts"], {
      env: { ...process.env, UHS_PLACE: place, UHS_YEAR: String(year) },
      stdio: "inherit",
    });
}
const out = ["# City panel", "", `Generated ${new Date().toISOString()}`, "", ...rows];
if (capture)
  out.push("", ...PANEL.map(({ place, year }) => `![${place} ${year}](${place}-${year}.png)`));
