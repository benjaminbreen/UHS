...
for family in catalog['families']:
    for variant in range(len(family['variants'])):
        ...
        if family['id'] in ['hearth','oil-lamp']:image.info['shadowMinY']=30
...
d.text((24,17),'UHS / 40 SHARED PROP FAMILIES / ORIGINAL PIXEL STUDIES / 3x NATIVE PIXELS',fill='#e7d5ad')
for index,family in enumerate(catalog['families']):
    ...
    shadow=shadows[f'morning:{key}'];ax,ay=shadow.info['anchor']
    shadow=shadow.resize((shadow.width*3,shadow.height*3),Image.Resampling.NEAREST)
    proof.paste(shadow,(round(x+74-ax*3),round(y+144-ay*3)),shadow)
