...
await mkdir('artifacts/street-review',{recursive:true});
const cards:string[]=[];
try{for(const [name,query] of [
 ...
 await page.locator('canvas').screenshot({path:`artifacts/street-review/${name}.png`});
 await page.evaluate(()=>{const lab=(window as any).terrainLab;lab.runtime.zoom=2;lab.scene.draw();});
 await page.waitForFunction(()=>document.querySelector('canvas')?.getAttribute('data-terrain-pending')==='0',{}, {timeout:90000});
 await page.locator('canvas').screenshot({path:`artifacts/street-review/${name}-detail.png`});
...
await writeFile('artifacts/street-review/index.html',`<!doctype html><meta charset="utf-8"><title>Street review</title><style>body{background:#252e24;color:#eee9d2;font:16px system-ui;margin:36px}img{width:100%;image-rendering:pixelated}main{display:grid;grid-template-columns:1fr 1fr;gap:24px}a{color:#ced9a9}</style><h1>Continuous footpaths and historical paving</h1><main>${cards.join('')}</main>`);
