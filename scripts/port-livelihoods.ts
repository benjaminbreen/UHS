...
/* Rank: where the work placed someone locally. Authored in UHS; the ported
 * table has no notion of standing at all. */
const RANKS = JSON.parse(
  readFileSync("scripts/data/livelihood-ranks.json", "utf8"),
) as Record<string, string | string[]>;
const rankOf = new Map<string, string>();
for (const [rank, ids] of Object.entries(RANKS))
  if (Array.isArray(ids)) for (const id of ids) rankOf.set(id, rank);
for (const id of rankOf.keys())
  if (!roles.has(id)) console.error(`rank for unknown role ${id}`);
