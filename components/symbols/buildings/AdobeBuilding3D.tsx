...
const AdobeBuilding3D: React.FC<AdobeBuilding3DProps> = React.memo(({ x, y, width, height, size, seed, tile }) => {
    const rand = new ValueNoise(seed + tile.x * 13 + tile.y * 31).random;
    const stories = 1 + Math.floor(rand() * 2.5);
    const elements: JSX.Element[] = [];
    const uniqueId = `adobe-${tile.x}-${tile.y}`;

    const baseColor = `hsl(28, 45%, ${75 + rand() * 10}%)`;
    ...
    for (let i = 0; i < stories; i++) {
        const storyWidth = width * (1 - i * 0.25) * (0.9 + rand() * 0.2);
        const storyHeight = (height / stories) * (1.2 - i * 0.15) + (i > 0 ? rand() * 4 - 2 : 0);
        const storyX = x + (width - storyWidth) / 2 + (i > 0 ? (rand() - 0.5) * width * 0.1 : 0);
        ...
        // Vigas (beams)
        for (let j = 0; j < 3 + rand()*2; j++) {
            if (rand() > 0.2) {
              const vigaY = storyY + 2 + rand() * 4;
              elements.push(<circle key={`viga-${i}-${j}`} cx={storyX + storyWidth + sideDepth * (0.2 + rand()*0.6)} cy={vigaY - sideDepth*0.5} r={size*0.03} fill={woodColor} stroke="#000" strokeWidth="0.2" opacity="0.8"/>)
            }
        }

        // Ladder if not ground floor and random chance
        if (i > 0 && rand() > 0.3) {
            const ladderX = storyX - size * 0.1 * (rand() > 0.5 ? 1 : -1) + (rand() > 0.5 ? storyWidth : 0);
            const ladderY = storyY + storyHeight * 0.1;
            const ladderHeight = (height / stories) + 5 + rand()*3;
        ...
        if (i === 0) {
            const doorHeight = storyHeight * 0.6;
            const doorWidth = storyWidth * 0.2;
            elements.push(<rect key={`door-${i}`} x={storyX + storyWidth/2 - doorWidth/2} y={storyY + storyHeight - doorHeight} width={doorWidth} height={doorHeight} fill="#2c1e12" stroke={outlineColor} strokeWidth="0.3" rx="0.5"/>);
        } else if (rand() > 0.5) { // Small windows on upper floors
            const winX = storyX + storyWidth * (0.2 + rand() * 0.6);
            const winY = storyY + storyHeight * (0.3 + rand() * 0.4);
        ...
        if (i === stories - 1) {
            if (rand() > 0.6) { // Pottery
                const potX = storyX + storyWidth * 0.2 + rand() * storyWidth * 0.6;
                elements.push(<ellipse key={`pot-${i}`} cx={potX} cy={storyY-2} rx={2} ry={2.5} fill={potteryColor} stroke="#000" strokeWidth="0.2" />);
            }
            if (rand() > 0.7) { // Chili ristra
                 elements.push(<rect key={`chili-${i}`} x={storyX + storyWidth-5} y={storyY+2} width={2} height={8} fill={chiliColor} />);
            }
        }
    }
