...
const NewItemModal: React.FC<NewItemModalProps> = ({ item, onClose }) => {
    return (
        <div
            className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-50 animate-panelSlideUp flex items-center justify-center p-4 border-t-2 border-amber-400/50 shadow-glow-amber"
            onClick={onClose}
        >
            <div className="flex items-center gap-6 text-white text-center">
                <div className="w-24 h-24 animate-jump flex items-center justify-center">
                    <GenerativeItemIcon item={item} size={96} />
                </div>
                <div className="text-left">
                    <p className="text-xs text-amber-300 font-semibold">ITEM ACQUIRED</p>
                    <h3 className="text-2xl font-bold">{item.name} {item.quantity > 1 ? `(x${item.quantity})` : ''}</h3>
