...
const BeliefsPanel: React.FC<BeliefsPanelProps> = ({ character, dense = false }) => {
  if (!character || !character.ideology) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-text-secondary surface-card border border-[var(--border-normal)] rounded-xl">
  ...
  return (
    <div className="h-full flex flex-col surface-card border border-[var(--border-normal)] rounded-xl overflow-hidden">
      <div className={`flex-1 min-h-0 ${dense ? 'p-2' : 'p-3'} overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800/50 space-y-3`}>
        ...
        {/* Core Ideology */}
        <section className={`rounded-2xl border border-amber-400/20 ${sectionPad} surface-muted`}>
          <h4 className={`font-bold tracking-wider text-amber-300 uppercase ${titleSize} pb-2 border-b border-amber-400/20`}>
            Core Ideology
          </h4>
          <div className={`mt-3 rounded-xl surface-muted border border-[var(--border-normal)] ${cardPad}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className={`italic text-text-secondary ${bodySize} leading-relaxed`}>
        ...
        {/* Personal Beliefs */}
        <section className={`rounded-2xl  ${sectionPad} surface-muted`}>
            ...
            {personalBeliefs.length > 0 ? (
              personalBeliefs.map(b => {
                ...
                return (
                  <div
                    key={b.id}
                    className={`rounded-xl border border-[var(--border-normal)] surface-muted shadow-sm ${cardPad}`}
                  >
                    <div className="flex items-start gap-3">
                      <div className="w-5 h-5 mt-0.5 shrink-0 text-text-secondary">
                        <Icon className="w-5 h-5" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className={`text-text-primary font-semibold leading-snug ${dense ? 'text-[14px]' : 'text-base'}`}>
                          {b.text}
                        </div>
                        ...
                        <div className="mt-3">
                          <div className="flex items-center justify-between text-text-secondary">
                            ...
                            <span className={`${dense ? 'text-[12px]' : 'text-sm'} font-bold text-text-primary`}>{b.conviction}%</span>
                          </div>
                          <div className="h-2 mt-1 rounded-full bg-[var(--surface-track-bg)] overflow-hidden">
                            <div
              ...
              <div className={`rounded-xl border border-[var(--border-normal)] surface-muted ${cardPad} text-center`}>
                <Sparkles className="w-6 h-6 mx-auto mb-2 opacity-60" />
                <p className={`italic text-text-tertiary ${dense ? 'text-[12px]' : 'text-sm'}`}>
