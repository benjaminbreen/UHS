...
const BeliefsPanel: React.FC<BeliefsPanelProps> = ({ character }) => {
    if (!character || !character.ideology) {
        return (
          <div className="flex flex-col items-center justify-center h-full text-gray-400 bg-slate-800/60 border border-slate-600/50 rounded-xl">
            ...
            <p className="text-lg font-semibold mb-2">No Beliefs Yet</p>
            <p className="text-sm opacity-75 text-center max-w-xs">Your character's beliefs and ideology will develop through gameplay.</p>
          </div>
        );
    }
    ...
    return (
        <div className="h-full flex flex-col bg-slate-800/60 border border-slate-600/50 rounded-xl overflow-hidden">
            <div className="flex-1 min-h-0 p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-slate-800/50 space-y-6">
                {ideology && (
                    <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/50">
                        <h4 className="pb-2 mb-3 text-sm font-bold tracking-wider text-amber-300 uppercase border-b border-amber-400/30 flex items-center gap-2">
                            Core Ideology
                        </h4>
                        <div className="p-3 rounded-lg bg-slate-900/40 border border-slate-700/30">
                            <h5 className="font-semibold text-lg text-amber-400 mb-2">{ideology.name}</h5>
                            <p className="text-sm italic text-slate-300 leading-relaxed">{ideology.description}</p>
                        </div>
                    </div>
                )}
                
                <div className="bg-slate-800/40 rounded-xl p-4 border border-slate-700/50">
                    <h4 className="pb-2 mb-3 text-sm font-bold tracking-wider text-blue-300 uppercase border-b border-blue-400/30 flex items-center gap-2">
                        Personal Beliefs
                    </h4>
                    <div className="space-y-4">
                        {personalBeliefs.length > 0 ? personalBeliefs.map(belief => (
                            <div key={belief.id} className="p-3 rounded-xl bg-slate-700/30 border border-slate-600/40 hover:bg-slate-700/50 transition-colors duration-200">
                                <p className="font-semibold text-slate-200 flex items-center gap-3 mb-3">
                                    <span className="text-lg">{belief.icon}</span>
                            ...
                            <div className="text-center py-8">
                                ...
                                <p className="italic text-slate-500">No strong personal beliefs recorded yet.</p>
                                <p className="text-xs text-slate-600 mt-2">Beliefs develop through your actions and choices.</p>
                            </div>
                        )}
                    </div>
                </div>
