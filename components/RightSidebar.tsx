...
const RightSidebar: React.FC = () => {
    ...
    const statusText = useMemo(() => {
        if (!playerCharacter) return 'Feeling okay';
                            ...
                            {SKILL_BUTTON_ORDER.map(skillId => {
                                 ...
                                 return (
                                    <button key={skillId} onClick={() => onUseSkill(skillId)} 
                                        className="flex flex-col items-center justify-center p-2.5 text-xs font-semibold text-gray-300 transition-all duration-200 border rounded-xl aspect-square bg-gradient-to-br from-slate-700/80 to-slate-800/60 border-gray-600/50 hover:bg-gradient-to-br hover:from-slate-600/90 hover:to-slate-700/70 hover:border-blue-400/50 hover:text-white hover:shadow-lg hover:-translate-y-1 hover:scale-105"
                                        title={skill.description}>
                                        <div className="mb-1 text-lg">{skill.icon}</div>
                                        <span className="text-[10px] leading-tight text-center">{skill.name}</span>
                                    </button>
                                )
                            })}
                        </div>
